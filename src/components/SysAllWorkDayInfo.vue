...
methods:{
    ...
    editWorkHoursInfo(input){
        this.sysEditWorkHoursInfoId = input.target.value;
        const sysEditWorkHoursInfoId = this.sysEditWorkHoursInfoId; // 保存 sysEditWorkHoursInfoId
        setTimeout(()=>{
            this.$router.push({ name: 'SysEditEmployeeWorkHoursInfo', params: { sysEditWorkHoursInfoId } });
        },0)
    }
},
watch:{
                    ...
                    <div class="infoFrame" id="infoFrame">
                        ...
                        <div class="cardFrame" id="cardFrame" v-dragscroll.x>
                            <div :style="{backgroundColor : hasntBeenApproved ? '' : 'rgba(220, 220, 220, 0.4)'}" class="workHoursInfoCard" v-for="(workHoursInfo , index) in selectedDateInfoList">
                                ...
                                <button v-if="hasntBeenApproved" :value="workHoursInfo.workInfoId" @click="editWorkHoursInfo" class="editWorkHoursInfo" id="sysEditEmployeeWorkHoursInfo">{{commitBtn}}</button>
                                <div v-if="!hasntBeenApproved" class="hasBeenApproved"><i class="fa-solid fa-check"></i>{{approvedText}}</div>
                            </div>
                            <div v-if="selectedDateInfoList.length > 1" class="tips"><i :style="{ transform : isAnimating ? 'rotate(-15deg)' : 'rotate(30deg)' }" class="fa-solid fa-hand"></i>{{dragText}}</div>
                        </div>
                    </div>
                    <button @click="backToWorkDayInfo" class="backToDayList" id="backToDayList">{{ backToAllText}}</button>
                </div>
                        ...
                        <div v-for="(workDayInfo , index) in workDayInfoList" :value="workDayInfo.worInfoId" :key="index" class="accordion-item">
                            ...
                            <div :id="'flush-collapse' + index" class="accordion-collapse collapse"
                                ...
                                <div class="accordion-body">
                                    <!-- 手風琴內容區 -->
                                    <p>{{ emploIdText }}: {{ workDayInfo.employeeId }}</p>
